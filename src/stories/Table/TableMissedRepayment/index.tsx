...
interface MissedRepaymentDataProps {
  id: number;
  loanId: string;
  originalDeductionDate: string;
...
interface TableMissedRepaymentProps {
  data: MissedRepaymentDataProps[];
  onClick?: (path: string) => void;
}

export const TableMissedRepayment: React.FC<TableMissedRepaymentProps> = (props) => {
      ...
      <table className='w-full table-auto'>
        <thead>
          ...
          >
            ...
            <th className='px-2'>Original Deduction Date</th>
            <th className='px-2'>Repayment Method</th>
            <th className='px-2'>Repayment Amount</th>
            <th className='px-2'>Missed</th>
            <th className='px-2'>Late Payment Fee</th>
            <th className='px-2'>Total Overdue Amount</th>
        ...
        <tbody>
          {tableData.map((item, index) => (
            ...
            >
              ...
              <td className='px-2'>{item.originalDeductionDate}</td>
              <td className='px-2'>{item.repaymentMethod}</td>
              <td className='px-2'>${item.repaymentAmount.toFixed(2)}</td>
              <td className='px-2'>{item.missed}</td>
              <td className='px-2'>${item.latePaymentFee.toFixed(2)}</td>
              <td className='px-2'>${(item.repaymentAmount + item.latePaymentFee).toFixed(2)}</td>
                ...
                <div className='w-full flex justify-center items-center'>
                  <div className='w-20'>
                    <OutlineButton
                      title='Pay Now'
                      fullWidth
                      onClick={() => onClick('/dashboard/pay-now')}
                      size='h22'
                    />
                  </div>
                </div>
